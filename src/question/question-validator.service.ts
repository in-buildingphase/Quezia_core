...
export class QuestionValidatorService {
    ...
    validateIdentity(question: QuestionPayload): void {
        if (!question.questionId || typeof question.questionId !== 'string' || question.questionId.trim() === '') {
            throw new BadRequestException('Question must have a valid non-empty questionId');
        }
        ...
        if (
            ...
            (typeof question.defaultTimeSeconds !== 'number' || question.defaultTimeSeconds <= 0)
        ) {
            throw new BadRequestException('defaultTimeSeconds must be a positive integer');
        }
    }
    ...
    validateMetadata(question: QuestionPayload): void {
        ...
        if (!question.subtopic || typeof question.subtopic !== 'string' || question.subtopic.trim() === '') {
    ...
    private validateMcq(question: QuestionPayload): void {
        ...
        if (!payload || !payload.question || typeof payload.question !== 'string' || payload.question.trim() === '') {
            throw new BadRequestException('MCQ contentPayload.question must be a non-empty string');
        }

        if (!Array.isArray(payload.options)) {
            throw new BadRequestException('MCQ contentPayload.options must be an array');
        }
        ...
        for (let i = 0; i < payload.options.length; i++) {
            ...
            if (!opt || typeof opt !== 'object') {
                throw new BadRequestException(`Option at index ${i} must be an object with {key, text}`);
            }
            ...
            if (seenKeys.has(normalizedKey)) {
                throw new BadRequestException(`Duplicate option key "${normalizedKey}" found`);
            }
            seenKeys.add(normalizedKey);

            if (!opt.text || typeof opt.text !== 'string' || opt.text.trim() === '') {
                throw new BadRequestException(`Option "${normalizedKey}" must have a non-empty text value`);
            }
        }
        ...
        if (!question.explanation || typeof question.explanation !== 'string' || question.explanation.trim() === '') {
            throw new BadRequestException('MCQ question must have a non-empty explanation');
        }
    }
    ...
    private validateNumeric(question: QuestionPayload): void {
        ...
        if (!payload || !payload.question || typeof payload.question !== 'string' || payload.question.trim() === '') {
            throw new BadRequestException('NUMERIC contentPayload.question must be a non-empty string');
        }

        // options must be absent
        if ((payload as any).options !== undefined) {
            throw new BadRequestException('NUMERIC question must not contain options');
        }
        ...
        if (!question.explanation || typeof question.explanation !== 'string' || question.explanation.trim() === '') {
            throw new BadRequestException('NUMERIC question must have a non-empty explanation');
        }

        if (question.numericTolerance === undefined || question.numericTolerance === null) {
            throw new BadRequestException('NUMERIC question must have numericTolerance defined (use 0 for exact match)');
        }

        const tolerance = this.safeParseDecimal(question.numericTolerance, 'numericTolerance');
        if (tolerance < 0) {
        ...
        if (Math.abs(parsedAnswer) > 0 && tolerance > Math.abs(parsedAnswer) * 100) {
            throw new BadRequestException(
    ...
    // ─────────────────────────────────────────────────────────────────────────
    safeParseNumericAnswer(value: string | number | null | undefined): number | null {
    ...
    validateAgainstTestSnapshot(
        ...
        const section = sectionSnapshot.find((s: any) => s.sectionId === targetSectionId);
        if (!section) {
            throw new BadRequestException(
        ...
        if (question.subject.trim().toLowerCase() !== section.subject.trim().toLowerCase()) {
            throw new BadRequestException(
        ...
        if (section.marksPerQuestion !== undefined && section.marksPerQuestion !== null) {
            const questionMarks = this.safeParseDecimal(question.marks, 'marks');
            const expectedMarks = this.safeParseDecimal(section.marksPerQuestion, 'marksPerQuestion');
        ...
        if (ruleSnapshot && ruleSnapshot.maxTimePerQuestion && question.defaultTimeSeconds) {
    ...
    private safeParseDecimal(value: any, fieldName: string): number {
        ...
        const str = String(value).trim();
        if (str === '') throw new BadRequestException(`${fieldName} cannot be empty`);

        // Allow: integers, decimals, negatives, scientific notation
        const decimalRegex = /^[+-]?(\d+\.?\d*|\.\d+)([eE][+-]?\d+)?$/;
        if (!decimalRegex.test(str)) {
            throw new BadRequestException(`${fieldName} "${value}" is not a valid decimal number`);
        }
        const n = Number(str);
        if (!isFinite(n)) {
            throw new BadRequestException(`${fieldName} "${value}" is not a finite number`);
        }
        return n;
    }
