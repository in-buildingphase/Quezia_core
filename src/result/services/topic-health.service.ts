...
export class TopicHealthService {
    ...
    analyzeTopicHealth(
        ...
        this.logger.log(`Analyzing topic health: Accuracy=${currentPerformance.accuracy.toFixed(2)}%, Attempts=${currentPerformance.attempts}`);
        ...
        const totalAttempts = historicalPerformances.reduce((sum, p) => sum + p.attempts, 0) + currentPerformance.attempts;
        ...
        // ────────────────────────────────────────────────────────────────────
        const daysSinceLastAttempt = this.getDaysSince(currentPerformance.timestamp);
        ...
        // ────────────────────────────────────────────────────────────────────
        const healthScore = accuracyScore + volumeScore + trendScore + consistencyScore + recencyScore + difficultyBalanceScore;
        ...
        this.logger.log(`Topic health analysis complete: Status=${healthStatus}, Score=${healthScore.toFixed(1)}/100`);
    ...
    private computeTrendScore(
        ...
        // Sort by timestamp
        const sorted = [...historical, current].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
        ...
        const recentAvg = recentHalf.reduce((sum, p) => sum + p.accuracy, 0) / recentHalf.length;
        const olderAvg = olderHalf.reduce((sum, p) => sum + p.accuracy, 0) / olderHalf.length;
     ...
     */
    private computeDifficultyBalance(
        ...
        const easyAccuracy = performance.easyTotal > 0 
            ? (performance.easyCorrect / performance.easyTotal) * 100 
            : 0;
        const mediumAccuracy = performance.mediumTotal > 0 
            ? (performance.mediumCorrect / performance.mediumTotal) * 100 
            : 0;
        const hardAccuracy = performance.hardTotal > 0 
        ...
        } else if (easyAccuracy >= mediumAccuracy || mediumAccuracy >= hardAccuracy) {
        ...
        const difficultyBalance = hasCoverage > 0 
            ? (easyAccuracy + mediumAccuracy + hardAccuracy) / (hasCoverage * 100) * 100 
            : 0;
    ...
    private generateRecommendations(metrics: any): string[] {
        ...
        if (metrics.easyAccuracy < 70) {
            recommendations.push('Strengthen basics before attempting harder questions');
        }
