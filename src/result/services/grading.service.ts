...
export class GradingService {
     ...
     */
    async calculateAttemptScore(attemptId: string): Promise<GradingResult | null> {
        ...
        this.logger.log(`Grading attempt ${attemptId} with ${attempt.questions.length} questions`);
        ...
        const defaultNegativeMarkValue = new Prisma.Decimal(ruleSnapshot?.negativeMarkValue || 0);
        ...
        for (const aq of attempt.questions) {
            ...
            if (!subjectBreakdown[subject]) {
                subjectBreakdown[subject] = { correct: 0, attempted: 0, total: 0, accuracy: 0, score: new Prisma.Decimal(0) };
            }
            const topicKey = `${subject}:${topic}`;
            if (!topicBreakdown[topicKey]) {
                topicBreakdown[topicKey] = { subject, topic, correct: 0, attempted: 0, total: 0, accuracy: 0, score: new Prisma.Decimal(0) };
            }
            if (!difficultyBreakdown[difficulty]) {
                difficultyBreakdown[difficulty] = { correct: 0, attempted: 0, total: 0, accuracy: 0, score: new Prisma.Decimal(0) };
            }
            ...
            if (questionType === 'MCQ') {
                ...
                const tolerance = tq.tolerance ? new Prisma.Decimal(tq.tolerance).toNumber() : 0;
            ...
            subjectBreakdown[subject].score = subjectBreakdown[subject].score.plus(marksAwarded);
            topicBreakdown[topicKey].score = topicBreakdown[topicKey].score.plus(marksAwarded);
            difficultyBreakdown[difficulty].score = difficultyBreakdown[difficulty].score.plus(marksAwarded);
        }
        ...
        const incorrectRate = totalQuestions > 0 ? incorrectCount / totalQuestions : 0;
        const unattemptedRate = totalQuestions > 0 ? unattemptedCount / totalQuestions : 0;
        ...
        this.logger.log(`Grading complete: Score=${totalScore}, Accuracy=${overallAccuracy.toFixed(2)}%, RiskRatio=${riskRatio.toFixed(2)}`);
