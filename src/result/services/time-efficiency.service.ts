...
interface TimeEfficiencyMetrics {
    ...
    averageTimeByDifficulty: Record<string, {
        averageTime: number;
        expectedTime: number;
        variance: number;
    ...
    // Subject time distribution
    subjectTimeDistribution: Record<string, {
        totalTime: number;
        averageTime: number;
        percentage: number;
    }>;
...
export class TimeEfficiencyService {
     ...
     */
    analyzeTimeEfficiency(questions: QuestionTimeData[], totalTime: number): TimeEfficiencyMetrics {
        this.logger.log(`Analyzing time efficiency for ${questions.length} questions`);
        
        // ────────────────────────────────────────────────────────────────────
        // 1. Average time by difficulty
        // ────────────────────────────────────────────────────────────────────
        const averageTimeByDifficulty = this.computeAverageTimeByDifficulty(questions);
        
        // ────────────────────────────────────────────────────────────────────
        // 2. Subject time distribution
        // ────────────────────────────────────────────────────────────────────
        const subjectTimeDistribution = this.computeSubjectTimeDistribution(questions, totalTime);
        
        // ────────────────────────────────────────────────────────────────────
        // 3. Time vs accuracy correlation
        // ────────────────────────────────────────────────────────────────────
        const timeAccuracyCorrelation = this.computeTimeAccuracyCorrelation(questions);
        ...
        this.logger.log(`Time efficiency analysis complete: Efficiency=${overallEfficiency.toFixed(1)}%, Inefficiency=${inefficiencyIndex.toFixed(2)}`);
     ...
     */
    private computeAverageTimeByDifficulty(questions: QuestionTimeData[]): Record<string, any> {
     ...
     */
    private computeSubjectTimeDistribution(questions: QuestionTimeData[], totalTime: number): Record<string, any> {
        ...
        for (const [subject, times] of Object.entries(subjectTime)) {
            ...
            const percentage = totalTime > 0 ? (totalSubjectTime / totalTime) * 100 : 0;
    ...
    private computeTimeAccuracyCorrelation(questions: QuestionTimeData[]): any {
        ...
        const times = questions.map(q => q.timeSpent || 0).filter(t => t > 0).sort((a, b) => a - b);
        ...
        const fastQuestions = questions.filter(q => q.timeSpent > 0 && q.timeSpent <= medianTime);
        const slowQuestions = questions.filter(q => q.timeSpent > medianTime);
        
        const fastCorrect = fastQuestions.length > 0 
            ? (fastQuestions.filter(q => q.isCorrect).length / fastQuestions.length) * 100 
            : 0;
        const slowCorrect = slowQuestions.length > 0 
            ? (slowQuestions.filter(q => q.isCorrect).length / slowQuestions.length) * 100 
            : 0;
    ...
    private computeInefficiencyIndex(
        ...
        const overthinkingPenalty = correlation < 0 ? overthinkingRate * 0.6 : overthinkingRate * 0.3;
        const rushingPenalty = correlation > 0 ? rushingRate * 0.6 : rushingRate * 0.3;
