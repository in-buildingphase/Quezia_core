...
export class RiskRatioService {
    ...
    computeRiskMetrics(questions: QuestionRiskData[]): RiskMetrics {
        ...
        const incorrectQuestions = attemptedQuestions.filter(q => q.isCorrect === false);
        const correctQuestions = attemptedQuestions.filter(q => q.isCorrect === true);
        ...
        // 2. Core rates
        // ────────────────────────────────────────────────────────────────────
        const attemptRate = totalQuestions > 0 ? (attemptedCount / totalQuestions) * 100 : 0;
        const skipRate = totalQuestions > 0 ? (unattemptedCount / totalQuestions) * 100 : 0;
        const incorrectRate = attemptedCount > 0 ? (incorrectCount / attemptedCount) * 100 : 0;
        ...
        // ────────────────────────────────────────────────────────────────────
        const negativelyMarkedQuestions = questions.filter(q => q.marksAwarded < 0);
        const negativeMarkingFrequency = attemptedCount > 0 
        ...
        this.logger.log(`Risk analysis complete: Ratio=${riskRatio.toFixed(2)}, Classification=${riskClassification}, NegativeImpact=${negativeMarkingImpact.toFixed(2)}`);
     ...
     */
    private computeSubjectRisk(questions: QuestionRiskData[]): Record<string, any> {
        ...
        for (const [subject, subjectQuestions] of Object.entries(subjectGroups)) {
            ...
            const incorrect = subjectQuestions.filter(q => q.attempted && q.isCorrect === false).length;
     ...
     */
    private computeDifficultyRisk(questions: QuestionRiskData[]): Record<string, any> {
        ...
        for (const [difficulty, diffQuestions] of Object.entries(difficultyGroups)) {
            const total = diffQuestions.length;
            const attempted = diffQuestions.filter(q => q.attempted).length;
            const unattempted = total - attempted;
            const incorrect = diffQuestions.filter(q => q.attempted && q.isCorrect === false).length;
    ...
    computeAggregateRiskRatio(historicalAttempts: { riskRatio: number }[]): {
        ...
        const averageRiskRatio = riskRatios.reduce((a, b) => a + b, 0) / riskRatios.length;
        
        // Compute trend (compare recent half vs older half)
        let riskTrend = 'STABLE';
        if (riskRatios.length >= 4) {
            const mid = Math.floor(riskRatios.length / 2);
            const recentAvg = riskRatios.slice(0, mid).reduce((a, b) => a + b, 0) / mid;
            const olderAvg = riskRatios.slice(mid).reduce((a, b) => a + b, 0) / (riskRatios.length - mid);
        ...
        const variance = riskRatios.reduce((acc, val) => {
